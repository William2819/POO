    ...
    ListarConsumoGen.prototype.listar = function () {
        ...
        filtrado = this.clientes.reduce(function (acc, cur) {
            cur.getProdutosConsumidos.forEach(function (p) {
                if (acc[cur.genero]["produto_".concat(p.getNome)]) {
                    acc[cur.genero]["produto_".concat(p.getNome)].qtd++;
                }
                else {
                    acc[cur.genero]["produto_".concat(p.getNome)] = { qtd: 1, nome: p.getNome };
                }
            });
            cur.getServicosConsumidos.forEach(function (s) {
                if (acc[cur.genero]["servico_".concat(s.nome)]) {
                    acc[cur.genero]["servico_".concat(s.nome)].qtd++;
                }
                else {
                    acc[cur.genero]["servico_".concat(s.nome)] = { qtd: 1, nome: s.getNomeServiço };
                }
            });
            return acc;
        }, { M: {}, F: {} });
        var ordenadoM = Object.keys(filtrado.M).map(function (k) {
            ...
            return { tipo: tipo, qtd: filtrado.M[k].qtd, nome: filtrado.M[k].nome };
        }).sort(function (a, b) { return b.qtd - a.qtd; }).slice(0, 3);
        var ordenadoF = Object.keys(filtrado.F).map(function (k) {
            var _a = k.split("_"), tipo = _a[0], nome = _a[1];
            return { tipo: tipo, qtd: filtrado.F[k].qtd, nome: filtrado.F[k].nome };
        }).sort(function (a, b) { return b.qtd - a.qtd; }).slice(0, 3);
